package com.example.liveplayer.view;
...
import com.example.liveplayer.rtsp.Client;
import com.example.liveplayer.rtsp.NavRtspCallBack;
...
public class PlayerView extends TextureView implements TextureView.SurfaceTextureListener, NavRtspCallBack {
    ...
    private void initRtsp() {
        livePool.execute(new Runnable() {
            @Override
            public void run() {
                client = new Client(PlayerView.this);
                client.play(url);
                Log.d(TAG, "initRtsp");
            }
        });
